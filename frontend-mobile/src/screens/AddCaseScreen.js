...
import {
  ...
  Modal, FlatList, StatusBar, Image,  // ✅ Added Image
} from 'react-native';
...
export default function AddCaseScreen({ navigation }) {
  ...
  const [woundPhoto, setWoundPhoto] = useState(null); // ✅ Added
  ...
  // ✅ Updated handleSubmit with FormData + image
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append('patientUserId', user?.id || user?._id || '');
    ...
    switch (step) {
      case 0: return (
        <>
          ...
          <Field label="Date of Birth * (MM/DD/YYYY)" placeholder="01/15/1990" value={dob} onChange={handleDobChange} colors={colors} />
          ...
          <Field label="Contact Number *" placeholder="09XX XXX XXXX" value={contact} onChange={setContact} keyboardType="phone-pad" colors={colors} />
      ...
      case 1: return (
        <>
          ...
          <Field label="Date of Exposure * (MM/DD/YYYY)" placeholder="01/15/2025" value={exposureDate} onChange={setExposureDate} colors={colors} />
          <Field label="Time of Exposure * (HH:MM)" placeholder="14:30" value={exposureTime} onChange={setExposureTime} colors={colors} />
      ...
      // ✅ Updated case 3 with image upload
      case 3: return (
        <>
          ...
          <View style={styles.inputGroup}>
            ...
            {woundPhoto ? (
              <View>
                <Image
                  source={{ uri: woundPhoto.uri }}
                  style={styles.previewImage}
                  resizeMode="cover"
                />
                <View style={styles.photoActions}>
                  <TouchableOpacity
                    style={[styles.photoBtn, { borderColor: colors.border, backgroundColor: colors.input }]}
                    onPress={handlePickImage}
                  >
                    <Text style={[styles.photoBtnText, { color: colors.text }]}>Change Photo</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.photoBtn, { borderColor: '#ef4444', backgroundColor: '#fef2f2' }]}
                    onPress={() => setWoundPhoto(null)}
                  >
              ...
              <View style={styles.uploadRow}>
                <TouchableOpacity
                  style={[styles.uploadBtn, { borderColor: colors.border, backgroundColor: colors.input }]}
                  onPress={handlePickImage}
                >
